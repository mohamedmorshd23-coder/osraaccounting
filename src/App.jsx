...
const firebaseConfig = {
  apiKey: "AIzaSyAnRlTKsgqATfshgkdDfWkIvqDvCjjo8XE",
  authDomain: "osraaccounting-3e47d.firebaseapp.com",
  projectId: "osraaccounting-3e47d",
  storageBucket: "osraaccounting-3e47d.firebasestorage.app",
  messagingSenderId: "477997500136",
  appId: "1:477997500136:web:8d904bd6444b97d98a6b5d",
};

const fbApp = getApps().length ? getApps()[0] : initializeApp(firebaseConfig);
const db = getFirestore(fbApp);
const COLLECTION = "clinicData"; // اسم الـ Collection في Firestore

/* ============================= STORAGE HELPERS ============================= */
// نظام التخزين: Firebase Firestore (المصدر الرئيسي) + localStorage (نسخة سريعة/احتياطية)
async function saveKey(key, value) {
  // احفظ فوراً في localStorage (استجابة سريعة + نسخة احتياطية محلية)
  try {
  ...
  // احفظ في Firebase (المصدر الدائم المشترك بين كل الأجهزة)
  try {
    await setDoc(doc(db, COLLECTION, key), {
      key, // نحتفظ بالمفتاح كحقل أيضاً للبحث بالـ prefix لاحقاً (مثلاً كل أيام شهر معيّن)
      value: JSON.stringify(value),
      updatedAt: new Date().toISOString(),
    });
  } catch (e) {
    console.error("Firebase save failed (سيتم الاعتماد على localStorage مؤقتاً):", e);
  }
}

// يبحث عن كل المفاتيح التي تبدأ بـ prefix معيّن (بديل window.storage.list)
...
async function loadKey(key, fallback) {
  // حاول التحميل من Firebase أولاً (البيانات الأحدث والمشتركة)
  try {
    const snap = await getDoc(doc(db, COLLECTION, key));
    if (snap.exists()) {
      const data = snap.data();
      if (data && data.value) {
        // حدّث النسخة المحلية لتكون متاحة بسرعة لاحقاً
  ...
  // إذا فشل Firebase (مثلاً بدون إنترنت)، استخدم النسخة المحلية
  try {
  ...
  const WelcomePage = () => (
    <div style={{ 
      ...
      <div style={{
        ...
        <h1 style={{ color: C.primary, fontSize: '2.5em', marginBottom: '10px', marginTop: 0 }}>
          أهلاً وسهلاً {session?.username || 'بك'}!
        </h1>
        <p style={{ color: C.text, fontSize: '1.2em', marginBottom: '30px', lineHeight: '1.8' }}>
          مرحباً بك في نظام إدارة حسابات العيادة<br/>
          ✨ تم تسجيل دخولك بنجاح<br/>
          📊 يمكنك الآن الوصول لجميع الميزات
        </p>
        <div style={{
          ...
          <p style={{ color: C.text, margin: 0, fontWeight: 'bold' }}>
            تم التطوير بواسطة د. محمد مرشد
          </p>
        </div>
        <button
...
if (!session) return <Login users={users} config={config} onLogin={(s) => { setSession(s); setShowWelcome(true); log("تسجيل دخول", `دخول ${s.username}`); }} />;

  if (session && showWelcome) return <WelcomePage />;
    ...
    <div dir="rtl" className="min-h-screen flex flex-col" style={{ background: C.bg, fontFamily: "'Cairo','Tajawal',sans-serif", color: C.text }}>
      <style>{`${FONT_IMPORT}
        * { direction: rtl; text-align: right; }
        ...
        /* Top nav scrolling */
        .nav-scroll { overflow-x: auto; scroll-behavior: smooth; }
        .nav-scroll::-webkit-scrollbar { height: 4px; }
        .nav-scroll::-webkit-scrollbar-thumb { background: ${C.border}; border-radius: 2px; }
      `}</style>
      {pinModal}

      {/* Top Bar with Logo */}
      <div className="no-print flex items-center justify-between px-3 py-2 sm:px-6 sm:py-4" style={{ background: C.surface, borderBottom: `1px solid ${C.border}` }}>
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="w-8 h-8 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center" style={{ background: C.primarySoft }}>
            <Stethoscope size={16} color={C.primary} className="sm:w-6 sm:h-6" />
          </div>
      ...
      {/* Top Navigation Bar - Icon Tabs */}
      <div className="no-print flex items-center gap-0.5 px-1 py-1 sm:px-2 sm:py-2 overflow-x-auto nav-scroll" style={{ background: C.surface, borderBottom: `1px solid ${C.border}` }}>
        {visiblePages.map(p => {
          const Icon = p.icon;
          ...
          return (
            <button
              ...
              className="flex flex-col items-center gap-1.5 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg transition-all active:scale-95 flex-shrink-0"
              title={p.label}
              style={{
                background: active ? C.primarySoft : "transparent",
                borderBottom: active ? `3px solid ${C.primary}` : "none",
                color: active ? C.primaryDark : C.textMuted,
                minWidth: "70px"
              }}>
              <Icon size={22} className="sm:w-6 sm:h-6 flex-shrink-0" />
              <span className="text-[11px] sm:text-sm font-semibold text-center leading-tight break-words w-full">{p.label}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="no-print flex items-center justify-between px-3 py-2 sm:px-6 sm:py-3" style={{ background: C.surface, borderTop: `1px solid ${C.border}` }}>
        <div className="text-xs sm:text-sm" style={{ color: C.textMuted }}>
          <span className="font-medium">{session.username}</span>
          {session.isAdmin && <span style={{ color: C.danger }}> (أدمن)</span>}
        </div>
        <button
          onClick={() => { log("تسجيل خروج", session.username); setSession(null); }}
          className="px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-all active:scale-95 flex items-center gap-1"
  ...
  return (
    <div>
      <PageHeader title="إدارة الأطباء" subtitle="إضافة وتعديل بيانات الأطباء ونسبهم"
        right={<div className="flex items-center gap-2"><Btn icon={Plus} onClick={openAdd}>إضافة طبيب</Btn><Btn variant="outline" icon={Printer} onClick={() => window.print()}>طباعة</Btn></div>} />
      <Card>
  ...
  return (
    <div>
      <PageHeader title="المصروفات" subtitle="تسجيل ومتابعة مصروفات العيادة حسب الفئة والتاريخ"
        right={<div className="flex items-center gap-2"><Btn icon={Plus} onClick={openAdd}>إضافة مصروف</Btn><Btn variant="outline" icon={Printer} onClick={() => window.print()}>طباعة</Btn></div>} />
      ...
      {!ready ? <EmptyState text="جاري التحميل..." /> : (
        <>
          ...
          <Card style={{ padding: 16 }}>
            <div className="font-bold text-sm mb-4">دخل الأطباء (من الأعلى للأقل)</div>
            {byDoctor.length === 0 ? <EmptyState text="لا يوجد بيانات في هذه الفترة" /> : (
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: C.bg }}>
                    <th className="text-right px-3 py-2 font-bold text-xs" style={{ color: C.textMuted }}>الترتيب</th>
                    <th className="text-right px-3 py-2 font-bold text-xs" style={{ color: C.textMuted }}>اسم الطبيب</th>
                    <th className="text-right px-3 py-2 font-bold text-xs" style={{ color: C.textMuted }}>إجمالي الدخل</th>
                  </tr>
                </thead>
                <tbody>
                  {byDoctor.map((d, i) => (
                    <tr key={d.name} style={{ borderTop: `1px solid ${C.border}` }}>
                      <td className="text-right px-3 py-2 text-xs font-semibold">{i + 1}</td>
                      <td className="text-right px-3 py-2 text-xs">{d.name}</td>
                      <td className="text-right px-3 py-2 text-xs font-bold" style={{ color: C.primary }}>{fmt(d.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function BackupPage({ doctors, expenses, salaries, users, auditLog, config, daysCache, setDaysCache, saveDoctors, saveExpenses, saveSalaries, saveUsers, saveConfig, log, monthKey }) {
    ...
    (async () => {
      try {
        ...
        for (let i = 0; i < 30; i++) {
          ...
          try {
            const backupData = await loadKey(`clinic:backup:${dateStr}`, null);
            if (backupData) {
              backups.push({
                date: dateStr,
  ...
  const exportAll = async () => {
    ...
    let allDaysKeys = [];
    try {
      allDaysKeys = await listKeysByPrefix("clinic:days:");
    } catch (e) {}
    const daysData = {};
